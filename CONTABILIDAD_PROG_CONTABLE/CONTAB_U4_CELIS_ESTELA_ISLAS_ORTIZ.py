...
def create_tables():
    ...
    try:
        ...
        cursor.execute("CREATE TABLE asientos2 (id INT(255), cargos VARCHAR(255), abonos VARCHAR(225), debe INT(255), haber INT(255))")
...
def sumas_as():
    try:
      ...
      sql="SELECT sum(debe) FROM asientos2 WHERE id = '{0}'".format(no_asiento.get())
      sql2="SELECT sum(haber) FROM asientos2 WHERE id = '{0}'".format(no_asiento.get())

      try:
          fcursor.execute(sql)
          conexion.commit()
          dat1=fcursor.fetchall()
      except:
          conexion.rollback()
      ...
      res1 = functools.reduce(lambda sub, ele: sub * 10 + ele, dat1)
      dat11=str(res1).replace(",","")
      dat11=str(dat11).replace("(", "")
      dat11=str(dat11).replace(")", "")
      print(dat11)

      res2 = functools.reduce(lambda sub, ele: sub * 10 + ele, dat2)
      dat22=str(res2).replace(",","")
      dat22=str(dat22).replace("(", "")
      dat22=str(dat22).replace(")", "")
      print(dat22)

      dat111 = (format(int(dat11), ',d'))
      dat222 = (format(int(dat22), ',d'))

      sql3="INSERT INTO asientos (id, cargos, abonos, debe, haber) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')".format(int(no_asiento.get()), "-**SUMAS**-",  "-**SUMAS**-", dat111 , dat222 )

      try:
          fcursor.execute(sql3)
          conexion.commit()
          dat2=fcursor.fetchall()
      except:
          conexion.rollback()
      ...
      for i in datos:
          lista_asientos.insert("", 0, text=i[0], values=(i[1], i[2], i[3], i[4],))
        
      no_asiento.config(state="readonly")
    except Exception:
        messagebox.showerror(title="AVISO", message="Error, todavia no se ha indicado un asiento")
        return
...
def add_as():
    ...
    if aboncar_box.get()=="CARGOS":
        ...
        sql2 = "INSERT INTO asientos2 (id, cargos, abonos, debe, haber) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')".format(no_asiento2, asiento_entry.get(), "",  int(importe.get()), 0)

    elif aboncar_box.get()=="ABONOS":
        sql="INSERT INTO asientos (id, cargos, abonos, debe, haber) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')".format(no_asiento2, "",  asiento_entry.get(), "", importe2)
        sql2 = "INSERT INTO asientos2 (id, cargos, abonos, debe, haber) VALUES ('{0}', '{1}', '{2}', '{3}', '{4}')".format(no_asiento2, asiento_entry.get(), "", 0, int(importe.get()))
...
def ven_asientos():
    ...
    boton_prueb=ttk.Button(ven3, text="Sumas de asiento", cursor="hand2", command=sumas_as)  
    boton_prueb.place(x=500, y=405, height=30,width=120)

...
def ven_asientos():
    ...
    ven3.config(bg="ghost white")
    w = int(1280)
    h = int(720)
    centrar(ven3, w, h)

    tabla_pedidos = LabelFrame(
        ven3, text=" ASIENTO DE DIARIO ")
    tabla_pedidos.grid(row=6, column=0, padx=20, pady=80)
    ...
    asiento_entry=ttk.Combobox(ven3, font=("calibri light", 9), state="readonly", values=[
        "Agregar las cuentas"])
    asiento_entry.place(x=0, y=10)

    cuenta_cod=Entry(ven3, background="lavender", justify="left", foreground="gray63", font=("calibri light", 12))
    cuenta_cod.place(x=0, y=30)

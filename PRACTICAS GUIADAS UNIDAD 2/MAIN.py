...
def guardar_alumno():
    ...
    conexion.execute(
        f"insert into alumnos(nombre, edad) values ('{nuevo.getNombre()}',{nuevo.getEdad()})")
    conexion.commit()
    conexion.close()
...
def nuevo_alumno(event=None):
    ...
    submit_button = tk.Button(ventana_nuevo_alumno,
                              text="Guardar", command=guardar_alumno)
...
menu_alumnos.add_command(label="Agregar Alumno",
                         accelerator="Ctrl+N", command=nuevo_alumno)
